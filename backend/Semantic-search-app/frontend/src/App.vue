      ...
      <div class="card-container">
        <div v-for="(result, index) in paginatedResults" :key="index" class="card">
          ...
          <div class="video-preview">
            <video :src="generateVideoLink(result.video_id, result.starttime, result.endtime)" controls></video>
          </div>
          <div class="card-content">
            ...
            <a :href="generateVideoLink(result.video_id, result.starttime, result.endtime)" class="video-link">Watch Video</a>
          </div>
        </div>
      </div>
...
export default {
  ...
  try {
    ...
    generateVideoLink(videoId, startTime, endTime) {
      return `/videos/${videoId}.mp4#t=${startTime},${endTime}`;
    },
